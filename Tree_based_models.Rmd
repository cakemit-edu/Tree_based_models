...
plotConfMat <- function(df) {
  df |> 
    ggplot(aes(x, y)) +
    geom_tile(aes(fill=gg.col), colour="white", alpha=.6) +
...
```{r}
class_and_probs_metrics <- metric_set(accuracy, roc_auc, f_meas, brier_class, 
...
tree_conf_matrix <- conf_mat(tree_test, truth=Survived, estimate=.pred_class) |> 
  tidy() |> 
  tidyConfMat() |> 
  mutate(model="DecTree0")

plotConfMat(tree_conf_matrix)
```
...
(tree_metrics <- tree_test |> 
    class_and_probs_metrics(truth=Survived, estimate=.pred_class, .pred_TRUE) |> 
...
With an overall accuracy of `r scales::percent(filter(tree_metrics, .metric=="accuracy")$.estimate, accuracy=1)`, the prediction results are not bad. The model worked particularly well in predicting passengers that did not survive (specificity = `r filter(tree_conf_matrix,category=="TN")$value`/(`r filter(tree_conf_matrix,category=="TN")$value`+`r filter(tree_conf_matrix,category=="FP")$value`) = `r scales::percent(filter(tree_metrics, .metric=="specificity")$.estimate)`). As you can see, specificity is higher than sensitivity. However, the difference is not big enough to consider the dataset as unbalanced.
...
```{r fig.asp=1, fig.width=2}
rf_conf_matrix <- conf_mat(rf_test, truth=Survived, estimate=.pred_class) |> 
  tidy() |> tidyConfMat() |> 
  mutate(model="RndFor0")

plotConfMat(rf_conf_matrix)
```
...
p2 <- autoplot(rf_roc_curve) +
  scale_x_continuous(expand=expansion(mult=c(0,0))) +
  scale_y_continuous(expand=expansion(mult=c(0,0))) +
  labs(title=glue::glue("Random Forest - AUC: {filter(rf_metrics, .metric=='roc_auc')$.estimate}"), 
       x="False Positive Rate (specificity)", y="True Positive Rate (sensitivity)")

p1 <- autoplot(tree_roc_curve) +
  scale_x_continuous(expand=expansion(mult=c(0,0))) +
  scale_y_continuous(expand=expansion(mult=c(0,0))) +
  labs(title=glue::glue("Decision Tree - AUC: {filter(tree_metrics, .metric=='roc_auc')$.estimate}"), 
       x="False Positive Rate (specificity)", y="True Positive Rate (sensitivity)")
...
This will loop over all the workflows in the `workflow_set` object, applying the `fit_resamples` function to each one, using the `df2_folds` cross-validation specifications. The results of each of these runs is stored under the respective model row in a new updated `workflow_set` object. 
...
Imagine we had many combinations of model specs and recipes with resampling. The `autoplot()` method plots the rankings; it also has a `select_best` argument. The plot in Figure \@ref(fig:wfs_results_plot) visualizes the best results for each model and is generated with:

```{r wfs_results_plot, fig.cap="*Model selection after resampling based on AUC performance*", fig.width=5, fig.asp=.5}
autoplot(
  wfs_results,
...
```{r}
best_test <- augment(best_fit, new_data=df1_test)

best_roc_curve <- best_test |> 
  roc_curve(truth=Survived, .pred_TRUE)

best_conf_matrix <- conf_mat(best_test, truth=Survived, estimate=.pred_class) |> 
  tidy() |> 
  tidyConfMat() |> 
  mutate(model="RndFor2")

best_metrics <- best_test |> 
  class_and_probs_metrics(truth=Survived, estimate=.pred_class, .pred_TRUE) |> 
  mutate(.estimate = round(.estimate, 4)) |> 
  mutate(model="RndFor2")
```
...
tree2_conf_matrix <- conf_mat(tree2_test, truth=Survived, estimate=.pred_class) |> 
  tidy() |> 
  tidyConfMat() |> 
  mutate(model="DecTree2")

tree2_metrics <- tree2_test |> 
  class_and_probs_metrics(truth=Survived, estimate=.pred_class, .pred_TRUE) |> 
  mutate(.estimate = round(.estimate, 4)) |> 
  mutate(model="DecTree2")
```
...
p4 <- autoplot(best_roc_curve) +
  scale_x_continuous(expand=expansion(mult=c(0,0))) +
  scale_y_continuous(expand=expansion(mult=c(0,0))) +
  labs(title=glue::glue("RF resampled - AUC: {filter(best_metrics, .metric=='roc_auc')$.estimate}"), 
       x="False Positive Rate (specificity)", y="True Positive Rate (sensitivity)")

p3 <- autoplot(tree2_roc_curve) +
  scale_x_continuous(expand=expansion(mult=c(0,0))) +
  scale_y_continuous(expand=expansion(mult=c(0,0))) +
  labs(title=glue::glue("DT resampled - AUC: {filter(tree2_metrics, .metric=='roc_auc')$.estimate}"), 
       x="False Positive Rate (specificity)", y="True Positive Rate (sensitivity)")
...
df_features <- rbind(
  ...
  ranger::importance(extract_fit_engine(best_fit)) |> tidy() |> 
    arrange(desc(x)) |> 
    mutate(model="RndFor2")
)
